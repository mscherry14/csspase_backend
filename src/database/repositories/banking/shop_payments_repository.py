...
class ShopPaymentsRepository:
    ...
    async def create_payment(self, payment: ShopPaymentDB) -> SimpleResult[ShopPaymentDB]:
        """
        ...
        unfinished_payment = await self.get_collection().find_one(
            {"fromUserBankingAccount": payment.fromUserBankingAccount,
             "status": TransferStatus.processing}) or await self.get_collection().find_one(
            {"orderId": payment.orderId, "status": TransferStatus.processing}) or await self.get_collection().find_one(
            {"orderId": payment.orderId, "status": TransferStatus.completed})
