...
class EventBankingAccountsRepository:
    ...
    async def create_account(self, event_id: str, init_balance: int) -> SimpleResult[EventBankingAccountDB]:
        user_doc = await self.get_collection().find_one({"event": event_id})
        if not user_doc:
    ...
    async def get_account_by_event_id(self, event_id: str) -> SimpleResult[EventBankingAccountDB]:
        user_doc = await self.get_collection().find_one({"event": event_id})
        if not user_doc:
    ...
    async def get_account_by_account_id(self, account_id: str) -> SimpleResult[EventBankingAccountDB]:
    ...
    async def commit_formed_transaction(self, account: EventBankingAccountDB, session: AgnosticClientSession | None):
