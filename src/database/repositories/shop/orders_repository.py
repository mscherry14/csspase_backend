...
class OrdersRepository(AsyncRepository[OrderDB]):
    ...
    async def orders_by_user_id(self, user_id: int, session: AsyncIOMotorClientSession | None) -> SimpleResult[List[OrderDB]]:
    ...
    async def order_by_order_id(self, order_id: str, session: AsyncIOMotorClientSession | None) -> SimpleResult[OrderDB]:
        try:
