...
class OrdersRepository(AsyncRepository[OrderDB]):
    ...
    async def orders_by_user_id(self, user_id: int) -> SimpleResult[List[OrderDB]]:
        res = await self.find_all({"recipientId": user_id})
        if isinstance(res, SimpleErrorResult):
            return SimpleErrorResult(message=res.message)
        else:
            return SimpleOkResult(payload=res.payload)

    async def order_by_order_id(self, order_id: str) -> SimpleResult[OrderDB]:
        try:
